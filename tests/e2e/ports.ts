...
export const parseE2ePortBase = (raw = process.env.BRIEF_E2E_PORT_BASE): number => {
  if (raw === undefined || raw === "") return E2E_PORT_BASE_DEFAULT;
  if (!/^\d+$/u.test(raw)) {
    throw new Error("BRIEF_E2E_PORT_BASE must be a decimal integer");
  }
  const value = Number(raw);
  if (!Number.isSafeInteger(value) || value < E2E_PORT_MIN || value > E2E_PORT_MAX) {
    throw new Error(
      `BRIEF_E2E_PORT_BASE must be a safe integer between ${E2E_PORT_MIN} and ${E2E_PORT_MAX}`,
    );
  }
  return value;
};
