...
export const InternalQueryPlanPrompt = [
  ...
  'Input inventory: Exactly {"question":string,"selectedConversation":Array<complete-entry | failed-entry>,"locale":string,"market":string,"currentTimestamp":string,priorValidationFeedback?:string}. priorValidationFeedback is one bounded code-owned enum value; use it only to correct the prior semantic output. Authorization, source names, limits, and physical stores remain code-owned and are never supplied as a model inventory.',
  'Output contract: Exactly {action:"skip",reason:string} or {action:"search",queries:Array<InternalQuery>}. Each query has purpose, a non-empty targets array, all/anyOf/not atoms, and one order. Every target is exactly {kind:"documents",filters:DocumentFilters} or {kind:"chat_messages",filters:ChatMessageFilters}; target kinds are unique and filters is required ({} means no filters). Do not add source IDs, SQL, limits, rank weights, cursors, scope, or legacy filters aliases.',
  internalQueryAtomRule,
  "Meaning: all atoms are required, each anyOf group requires one member, and not atoms exclude matches. A query searches exactly the listed targets. Use one documents target for document retrieval, one chat_messages target for older chat retrieval, and both target objects when the question needs both stores. Never omit a target or add one in code. A negative-only query must include a positive indexed date, source, language, type, or author filter for every listed target.",
  'Examples: Single-store document query uses targets:[{kind:"documents",filters:{}}]. Single-store older-chat query uses targets:[{kind:"chat_messages",filters:{}}]. Multi-store query uses targets:[{kind:"documents",filters:{}},{kind:"chat_messages",filters:{}}].',
  'Freshness: For a broad request such as "what\'s new", "quoi de neuf", or "depuis hier", emit exactly one documents target whose filters.publishedAt contains the bounds [currentTimestamp - 24 hours, currentTimestamp) (after=currentTimestamp - 24 hours, before=currentTimestamp), with order "newest" and empty all and anyOf arrays. Do not put generic words such as news, actualités, or nouveautés in query atoms. Keep subject atoms only when the user names a subject. The timestamp bounds, not freshness vocabulary, define the requested time range.',
...
export const InternalQueryReviewPrompt = [
  ...
  'Input inventory: Exactly {"question":string,"queries":Array<InternalQuery>,"results":Array<ReviewResult>,"coverage":Array<BranchCoverage>,"truncation":{branch:boolean,candidates:boolean,hydration:boolean},priorValidationFeedback?:string}. priorValidationFeedback is one bounded code-owned enum value; use it only to correct the prior semantic output. Result overviews contain only run-local result IDs, kind, label/date, exact full-content token count, exact preview, normalized fused score, matched query ordinals, coverage, and truncation flags. They contain no source IDs, message IDs, hashes, ranges, SQL, or table names.',
  ...
  internalQueryAtomRule,
  'Target form: Every replacement query has a non-empty unique targets array with {kind:"documents",filters:DocumentFilters} and/or {kind:"chat_messages",filters:ChatMessageFilters}; filters is required and {} means no filters.',
  'Examples: Single-store document query uses targets:[{kind:"documents",filters:{}}]. Single-store older-chat query uses targets:[{kind:"chat_messages",filters:{}}]. Multi-store query uses targets:[{kind:"documents",filters:{}},{kind:"chat_messages",filters:{}}].',
  "Review rules: Accept useful partial coverage. Replace only for a clear missed concept, narrow filter, wrong language, or unsupported branch. Do not loosen an explicit user constraint. Preserve exact target selection in every replacement: use one documents target, one chat_messages target, or both target objects as needed; never infer a missing target. For a broad freshness request, the replacement must contain exactly one documents target whose filters.publishedAt contains [currentTimestamp - 24 hours, currentTimestamp) (after=currentTimestamp - 24 hours, before=currentTimestamp), with order newest and empty all and anyOf arrays unless the user named a subject; do not add generic freshness words. If results cannot support the request, return no_evidence.",
  grounding,
  restrictedContent,
  structuredOutput,
].join("\n\n");
