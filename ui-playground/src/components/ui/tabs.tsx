...
export const TabsList = forwardRef<HTMLDivElement, React.ComponentPropsWithoutRef<typeof TabsPrimitive.List>>(
  ({ className, ...props }, ref) => (
    <TabsPrimitive.List
      ref={ref}
...
export const TabsTrigger = forwardRef<HTMLButtonElement, React.ComponentPropsWithoutRef<typeof TabsPrimitive.Trigger>>(
  ({ className, ...props }, ref) => (
...
export function Segmented<T extends string>({
  ...
  return (
    <div
      role="radiogroup"
      aria-label={ariaLabel}
      className={cn("inline-flex select-none rounded-tiny border border-line-2 bg-paper", className)}
      onKeyDown={(e) => {
    ...
    >
      {options.map((opt, i) => {
        ...
        return (
          ...
          >
            {opt.label ?? opt.value}
            <span className="sr-only">{selected ? ` (${t("common.selected")})` : ""}</span>
          </button>
        );
      })}
    </div>
