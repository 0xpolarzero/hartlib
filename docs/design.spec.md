...
The demo client chat is live. It reads chat history from the Hartlib API, sends messages through the Hartlib API, streams active AI runs over SSE, renders citations from inline citation tags, and shows the sources read for each assistant answer. Assistant turns use an unframed left answer column with type, spacing, and hairline rules; the assistant label keeps turn ownership clear without making a bot icon the main signal. User turns remain compact right-aligned bubbles. While an answer runs, the shared transcript shows one fixed five-slot rail from run start — Understanding, Evidence, Preparing, Writing, and Finishing — before the streamed answer text. A quiet 1px connector sits behind the markers, and only completed segments fill with the oxblood accent; each slot keeps its label and position while its waiting, running, complete, retrying, failed, or skipped state changes in place with both a glyph and text for non-color access. Future slots stay muted until activity reaches them. After a successful answer is saved, the rail is replaced by an unboxed localized “Answer ready” row with only the source-read and cited counts carried by the saved message, followed by the closed-by-default sources-read disclosure. Cited sources show a server-authorized supporting quote only inside that opened disclosure; every unavailable case uses “Quote unavailable for this source.” and the browser never derives quote text from ranges or source metadata. An owner-only, closed-by-default “Debug details” disclosure loads the bounded safe run projection only after opening and shows a clear unavailable state after retention expiry; shared viewers do not receive its loader. The existing active diagnostics disclosure still reveals only safe in-memory activity data. When the sources-read disclosure opens, cited sources use the same ordinal shown by inline citation markers and read-but-uncited sources are marked without a made-up number. The composer sends an explicit per-message web-search choice. The UI does not treat streamed answer completion as terminal until the required parallel memory write has committed and `done` arrives. A terminal failure discards the provisional assistant draft and remains reload-durable on the originating user message, with resubmit only when the run is retryable. Stream growth keeps the transcript anchored only when the viewer is near the bottom; after a manual scroll away, an accessible Jump to latest control returns to the newest content.
...
The client demo root centers on one chat and a compact Subscriptions table. Subscriptions include both seeded publisher invitation sources and real public sources ingested by the worker, unified as one source model. Public rows come from the demo company's authorized `client_company_public_source_settings`; globally ingested but unauthorized rows do not appear. Each subscription row shows the source name, source type, latest publication date when present, and an enabled switch. The switch controls whether chat can retrieve that source and does not change publication access. Its column header explains that chat searches only enabled subscriptions. Selecting a source replaces the table with that source's delivered publications, and selecting a publication opens its issue detail. Clickable breadcrumbs show `Subscriptions > source > issue` and let the user return to each ancestor. The demo uses source-level settings, not a per-chat source picker. Public-source subscriber, open, download, and AI-context-pull analytics remain null unless an authoritative persisted fact exists; the UI never fabricates zeroes.
...
The UI also shows a separate closed-by-default expandable sources-read view. It contains the source names and links for the answer context, preserves the server-provided source order, repeats inline citation ordinals for cited sources, and labels read-but-uncited sources without assigning them an ordinal. For cited sources, it renders the exact server-authorized quote when the citation record carries one; otherwise it renders the same generic “Quote unavailable for this source.” state without revealing why. The disclosure uses a native keyboard and screen-reader summary/list relationship.
