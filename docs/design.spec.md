...
The demo client chat is live. It reads chat history from the Hartlib API, sends messages through the Hartlib API, streams active AI runs over SSE, renders citations from inline citation tags, and shows the sources read for each assistant answer. Assistant turns use an unframed left answer column with type, spacing, and hairline rules; the assistant label keeps turn ownership clear without making a bot icon the main signal. User turns remain compact right-aligned bubbles. While an answer runs, the shared transcript shows one fixed five-slot rail from run start — Understanding, Evidence, Preparing, Writing, and Finishing — before the streamed answer text. A quiet 1px connector sits behind the markers, and only completed segments fill with the oxblood accent; each slot keeps its label and position while its waiting, running, complete, retrying, failed, or skipped state changes in place with both a glyph and text for non-color access. Future slots stay muted until activity reaches them. After a successful answer is saved, the rail is replaced by an unboxed localized “Answer ready” row with only the source-read and cited counts carried by the saved message, followed by the closed-by-default sources-read disclosure. An explicit diagnostics disclosure reveals only the safe activity history and run summary. When the sources-read disclosure opens, cited sources use the same ordinal shown by inline citation markers and read-but-uncited sources are marked without a made-up number. The composer sends an explicit per-message web-search choice. The UI does not treat streamed answer completion as terminal until the required parallel memory write has committed and `done` arrives. A terminal failure discards the provisional assistant draft and remains reload-durable on the originating user message, with resubmit only when the run is retryable. Stream growth keeps the transcript anchored only when the viewer is near the bottom; after a manual scroll away, an accessible Jump to latest control returns to the newest content.
...
The UI also shows a separate closed-by-default expandable sources-read view. It contains the source names and links for the answer context, preserves the server-provided source order, repeats inline citation ordinals for cited sources, and labels read-but-uncited sources without assigning them an ordinal. The disclosure uses a native keyboard and screen-reader summary/list relationship.
