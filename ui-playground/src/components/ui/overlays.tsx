...
>(({ className, ...props }, ref) => (
  <DropdownMenuPrimitive.Label ref={ref} className={cn("caps-label px-1.5 py-1.5 text-ink-2", className)} {...props} />
));
...
>(({ className, ...props }, ref) => (
  <DropdownMenuPrimitive.Portal>
    <DropdownMenuPrimitive.SubContent
      ref={ref}
      sideOffset={2}
      className={cn("z-50 min-w-40 rounded-tiny border border-line-2 bg-surface p-1 shadow-none data-[state=open]:animate-enter", className)}
      {...props}
    />
  </DropdownMenuPrimitive.Portal>
));
    ...
    <TooltipPrimitive.Root delayDuration={320}>
      ...
      <TooltipPrimitive.Portal>
        ...
        >
          {content}
          {shortcut && (
            <span className="font-mono text-[10px] text-paper/70">{shortcut}</span>
          )}
        </TooltipPrimitive.Content>
      </TooltipPrimitive.Portal>
