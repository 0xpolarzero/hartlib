...
Client company membership removal is a retained revocation, not a hard delete. `revoked_at` and `revoked_by_user_id` are set atomically with revocation of the employee's subscription grants; the row remains for chat, notification, preferences, usage, and other durable foreign keys. Every authorization query requires an unrevoked membership plus a live user and company. Member-authorized local mutations and sensitive reads serialize on `brief:client-members:<companyId>` so a concurrent removal either follows the already-accepted operation or commits first and makes its recheck fail. Chat share, unshare, and delete also row-lock the chat and take the chat execution lock before their final recheck. Issue-detail authorization and issue/document projection are one SQL statement, so a client never receives documents from a different authorization snapshot. Direct membership deletion is database-rejected outside the account-purge transaction, and only unrevoked live admins satisfy the last-admin invariant.

Chat lists acquire the exact sorted current client-membership lanes before their final single-statement projection. A full chat read row-locks the chat, then holds its client-membership lane, chat-execution lane, live requester row, and live company row through the final authorization recheck and all message/run/source queries. Demo `GET /v1/chat` first idempotently ensures its demo workspace, then performs that same authorized leased projection; it never calls an unleased message/run/source loader. Every worker terminal transition first resolves the run's immutable scope, then acquires the user-memory lane, chat row, client-membership lane, chat-execution lane, and run row in that canonical order. Concurrent success finalization therefore cannot insert an assistant message between projection queries, failure handling cannot change the projected run outcome between them, and revocation, unshare, chat deletion, user deletion, or company deletion linearizes before the read and produces no content or after the complete stable projection. Delivered issue detail takes the applicable sorted client/publisher membership lanes before its final atomic issue-and-document projection. Publisher PDF signing row-locks the issue/document, holds those same membership lanes plus live user/company rows, rechecks exact publisher or delivered-client access, and retains the lease through the abortable 20-second signing boundary and five-minute bearer URL issuance.
...
Production public-source authorization is deny-by-default per client company. `GET /v1/client-companies/:companyId/public-sources` lists the globally ingested catalog with the company's effective toggle; `PUT /v1/client-companies/:companyId/public-sources/:sourceId` accepts exactly `{ enabled: boolean }` and requires an MFA-verified company admin. An absent setting is disabled. The chat runtime joins enabled settings to the current chat company when loading its catalog and rechecks the setting before search exposure, hydration, frozen-context use, and finalization. The demo public-source read route resolves the deterministic demo company and projects only its enabled settings, so every displayed subscribed source shares the chat authorization boundary. Public-source subscriber/open/download/AI-context-pull fields are nullable and remain null until an authoritative persisted fact exists; zero placeholders are not valid projections.
