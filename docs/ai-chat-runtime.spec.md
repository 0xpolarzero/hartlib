...
Provider-template rendering suppresses the generation prompt when the request already ends in assistant text. GLM-5-Turbo's provider-only accounting adds one token for that trailing assistant continuation and one for every historical assistant tool-call turn at or after the latest assistant prose, including a mixed prose-and-tool-call turn. Earlier tool turns are absorbed by the provider's prose-boundary preprocessing. The opt-in live tokenizer contract covers each shape.
...
Repeated exact preview rows from distinct allowed search calls collapse to one source identity; they do not create false candidates, while any additional identity still fails the exact set. An authorized message from the seeded conversation that appears only as a non-selected internal search or inspection distractor is identity-attested and maps to no pulled golden source; serializing that distractor into an answer remains a source defect.
