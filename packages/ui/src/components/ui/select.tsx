...
/**
 * Radix-backed select ported from ui-playground. The legacy `className`
 * layout wrapper is honoured through a thin adapter; the visual tree (trigger,
 * portal content, items) is the reference verbatim.
 */
export function Select({
  className,
  ...props
}: React.ComponentPropsWithoutRef<typeof SelectPrimitive.Root> & { className?: string }) {
  if (className === undefined) return <SelectPrimitive.Root {...props} />;
  return (
...
export const SelectContent = forwardRef<
  HTMLDivElement,
  React.ComponentPropsWithoutRef<typeof SelectPrimitive.Content> & { ariaLabel?: string }
>(({ className, children, position = "popper", ariaLabel, ...props }, ref) => (
...
>(({ className, ...props }, ref) => (
  <SelectPrimitive.Label
    ref={ref}
    className={cn("caps-label px-1.5 py-1 text-ink-2", className)}
    {...props}
  />
));
