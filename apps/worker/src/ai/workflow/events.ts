...
export type AiRunEvent =
  ...
  | { readonly type: "text_delta"; readonly attempt: number; readonly delta: string }
  | {
...
export const appendAiRunEvent = (
  ...
  Effect.gen(function* () {
    ...
    const rows = yield* sql<NextSeqRow>`
      ...
      select ${aiRunId}, coalesce(max(seq), 0) + 1, ${sql.json(event)}
      from ai_run_events
...
export const appendAiRunEventOnce = (
  ...
  Effect.gen(function* () {
    ...
    return yield* appendAiRunEvent(aiRunId, event);
  });
