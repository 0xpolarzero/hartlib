...
Hartlib provides a real, durable AI chat over content the user is authorized to read.
...
One accepted user message creates one Hartlib `ai_runs` record, one queue job, and one Smithers run.

The browser talks only to the Hartlib API. It never calls Z.AI, Tinyfish, Pi, Smithers, provider tools, or Smithers tables.
...
Agents emit typed plans, queries, references, and text. They never emit SQL or receive database credentials. Hartlib code validates authorization, compiles parameterized SQL, fetches content, normalizes provenance, and performs product writes.
...
After a provider returns a reference, Hartlib code applies one kind-specific
...
| ------------------------------------------------------------------- | ---------------------------- | ------------------------------------------------------------------------------------------- |
| `load-turn`                                                         | Hartlib code                 | Stable run/request data and immutable acceptance scope.                                     |
| `plan-turn`                                                         | Hartlib code plus Pi         | One strict `clarify`/`single`/`fanout` route.                                               |
| `single-retrieve-internal` or `topic-tN-retrieve-internal`          | Hartlib code plus Pi         | One `InternalQueryPlan`, bounded parallel branch search, and one `QueryReview`/replacement. |
| `single-select-memories` or `topic-tN-select-memories`              | Hartlib code plus Pi         | Bounded memory selector result.                                                             |
| `single-retrieve-web` or `topic-tN-retrieve-web`                    | Hartlib code plus Pi         | Bounded web selector result.                                                                |
| `single-assemble` or `topic-tN-assemble`                            | Hartlib code                 | One immutable candidate ledger and context assembly.                                        |
| `single-measure`, `topic-tN-measure`, or `fanout-synthesis-measure` | Hartlib code                 | Exact provider-request fit measurement.                                                     |
| `*-compact-plan`                                                    | Hartlib code plus Pi         | One complete `CompactionPlan` after an overage.                                             |
| `*-compact-gNNN` / `*-fallback-gNNN`                                | Hartlib code plus Pi         | One complete group selection; groups run under the compaction semaphore.                    |
| `*-compact-collect` / `*-fallback-collect`                          | Hartlib code                 | Ordered group-result merge.                                                                 |
| `*-compact-measure` / `*-fallback-measure`                          | Hartlib code                 | Exact post-compaction measurement.                                                          |
| `*-fallback-plan`                                                   | Hartlib code plus Pi         | One monotone fallback manifest, at most once.                                               |
| `*-context-select`                                                  | Hartlib code                 | Final immutable context selection.                                                          |
| answer tasks, `answer-select`, `memory-extract`, `finalize`         | Hartlib code plus Pi or code | Grounded answer, memory proposal, and terminal product transaction.                         |
...
Hartlib owns chats, messages, `ai_runs`, events, source exposures, source maps,
...
Smithers pins Effect 3 and Hartlib backend code uses Effect 4. Smithers interop is isolated in the worker adapter whose Effect import resolves to Smithers' exact Effect 3 dependency. Smithers Effect values never cross the adapter into Hartlib Effect 4 services.

The approved development model provider is Z.AI through its official Coding Plan endpoint. Provider configuration remains behind the Hartlib model registry. The registry supplies the model's context window, maximum output, thinking capabilities, API format, exact tokenizer, and chat template.
...
Provider-template rendering suppresses the generation prompt when the request already ends in assistant text. GLM-5-Turbo's provider accounting ignores the `strict: false` metadata in each function definition; its pinned local counter subtracts the provider's four-token accounting omission per definition. The provider does not add a continuation or completed-tool-turn framing token. The opt-in live tokenizer contract covers zero-, one-, and three-function inventories and accumulated tool transcripts.

Z.AI reports a reused prompt prefix in `prompt_tokens_details.cached_tokens` while retaining those tokens inside `prompt_tokens`. Hartlib therefore stores the uncached prompt portion in `inputTokens`, the reused portion in `cachedTokens`, and compares the local exact prompt count with their sum. Reasoning tokens remain a subset of provider completion tokens and are recorded separately without being added to the local prompt comparison.
...
Smithers state and Hartlib product tables share one Postgres database but use separate tables. Output schema keys are namespaced. The API never reads or writes Smithers tables.

Hartlib stores references to internal content rather than copying that content into observations. Selected web quotations are stored with their URL and capture metadata because the remote page can change. Raw web pages, search previews, model prompts, and intermediate topic prose are not copied into durable product tables.
...
All model calls remain Smithers compute tasks whose async child invokes Pi. Hartlib does not use Smithers `agent=` execution. Hartlib async compute tasks do not use the Smithers `deps` prop: in installed 0.31.0, that shape invokes the function during render and treats the result as static. Components use structural ordering, optional `dependsOn` edges to nodes that are always mounted, and `ctx.output` or `ctx.outputMaybe` inside compute closures.
...
`hartlib_document_versions.language`, `hartlib_documents.media_type`,
...
revision IDs captured in the immutable acceptance scope. Hartlib code reads those
...
`documentId` binds to the logical `hartlib_documents.id` row,
`publisherDocumentId` binds that same publisher document inside its issue,
`snapshotId` binds `hartlib_document_versions.id`, and `publisherExtractionId`
binds the exact `hartlib_document_extractions.id` row that produced that version.
The forward migration adds a required one-to-one
`hartlib_document_versions.publisher_extraction_id` foreign key and uniqueness
...
`ready`, database constraints and triggers make the PDF, `hartlib_documents` row,
`hartlib_document_versions` row, `hartlib_document_extractions` row,
...
provider transport, Hartlib code creates an internal exposure-proof sidecar from
...
Immediately before each content-bearing provider request, Hartlib validates the
...
Direct and synthesized answers use the user's locale and Hartlib's editorial system prompt.
...
the strict `clarify`/`single`/`fanout` union at its own boundary. Hartlib validates
...
A fatal Smithers task failure stops the graph. The worker's failure handler marks the Hartlib run failed and appends a terminal `error` event idempotently. Smithers state is retained until that product failure transition succeeds, then cleaned up.
...
The demo reads persisted chat history, sends messages through the Hartlib API, reconnects to active streams, renders citation tags, and refreshes the saved message after `done`.
