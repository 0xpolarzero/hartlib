...
export type SheetProps = ComponentProps<typeof DialogPrimitive.Root> & { locale?: string };
export function Sheet({ locale: _locale, ...props }: SheetProps) {
...
export function SheetHeader({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div
      className={cn(
        "flex min-h-12 shrink-0 items-center justify-between gap-8 border-b border-line pr-10 pl-4",
        className,
      )}
      {...props}
    />
