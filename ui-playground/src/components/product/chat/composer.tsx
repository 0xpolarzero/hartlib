...
type DictationState = "idle" | "requesting" | "recording" | "processing" | "error";
...
const INITIAL_WAVEFORM = Array.from({ length: WAVEFORM_BAR_COUNT }, (_, index) => 0.25 + ((index * 7) % 5) * 0.04);
...
function errorForRecognition(error: string | undefined): DictationError {
  if (error === "not-allowed" || error === "service-not-allowed" || error === "permission-denied") return "permission";
...
export function Composer() {
  ...
  const startWaveform = (session: number, analyser: AnalyserNode) => {
    ...
    const update = (timestamp: number) => {
      if (!mountedRef.current || sessionRef.current !== session || analyserRef.current !== analyser) return;
      if (timestamp - lastWaveformUpdateRef.current >= 80) {
        ...
        const next = Array.from({ length: WAVEFORM_BAR_COUNT }, (_, index) => {
          ...
          const to = Math.max(from + 1, Math.floor(((index + 1) * data.length) / WAVEFORM_BAR_COUNT));
          let energy = 0;
          for (let cursor = from; cursor < to; cursor += 1) energy += Math.abs(data[cursor] - 128) / 128;
  ...
  const startDictation = async () => {
    if (streaming || (dictationStateRef.current !== "idle" && dictationStateRef.current !== "error")) return;
    const Recognition = recognitionConstructor();
    const AudioContext = audioContextConstructor();
    if (!Recognition || !AudioContext || typeof navigator === "undefined" || !navigator.mediaDevices?.getUserMedia) {
      sessionRef.current += 1;
    ...
    try {
      ...
      failDictation(session, name === "NotAllowedError" || name === "PermissionDeniedError" ? "permission" : "generic");
    }
  };
  ...
  const actionDisabled = dictation !== "idle";
      ...
      <div className="mx-auto max-w-[52rem]">
        ...
        <div className="flex items-end gap-2">
          {dictation === "idle" ? (
            ...
            <div role="alert" className="flex min-h-9 min-w-0 flex-1 items-center gap-2 rounded-tiny border border-danger/60 bg-danger-soft/50 px-3 py-1.5 text-[12px] text-danger">
              <span className="min-w-0 flex-1 truncate">{errorText}</span>
              <Button type="button" variant="ghost" size="sm" className="shrink-0 text-danger" onClick={startDictation}>
                {t("common.retry")}
              </Button>
              <Button type="button" variant="secondary" size="sm" className="shrink-0" onClick={dismissDictationError}>
                {t("composer.dictation.dismiss")}
              </Button>
            </div>
          ) : (
            <div
              className="flex min-h-9 min-w-0 flex-1 items-center gap-2 rounded-tiny border border-line-2 bg-surface px-2.5"
            >
              {dictation === "requesting" && (
                <>
                  <span className="sr-only" role="status" aria-live="polite">{t("composer.dictation.requesting")}</span>
                  <LoaderCircle aria-hidden="true" className="size-3.5 shrink-0 animate-spin-slow text-ink-2" />
                  <span className="truncate text-[12px] text-ink-2">{t("composer.dictation.requesting")}</span>
                </>
              )}
              {dictation === "recording" && (
                <>
                  ...
                  <span className="sr-only" role="status" aria-live="polite">{t("composer.dictation.listening")}</span>
                  ...
                  <time className="shrink-0 font-mono text-[11px] tabular-nums text-ink-2" dateTime={`PT${elapsedSeconds}S`}>
              ...
              {dictation === "processing" && (
                <>
                  <span className="sr-only" role="status" aria-live="polite">{t("composer.dictation.transcribing")}</span>
                  <LoaderCircle aria-hidden="true" className="size-3.5 shrink-0 animate-spin-slow text-ink-2" />
                  <span className="truncate text-[12px] text-ink-2">{t("composer.dictation.transcribing")}</span>
                </>
              )}
            </div>
          )}
        ...
        <div className="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1.5">
          ...
          <div className="ml-auto flex items-center gap-2">
            ...
            {streaming ? (
              <Button type="button" variant="secondary" size="icon" aria-label={t("composer.stop")} title={t("composer.stop")} onClick={chat.stop} className="border-ink">
