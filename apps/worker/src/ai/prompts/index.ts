...
export const WebResearchPrompt = [
  ...
  "Web-need rule: Use web tools only when this topic explicitly asks for current, latest, official, public, or otherwise web-verifiable information. If the topic is answerable without public web evidence, emit an empty manifest without calling web_search, even when the overall user turn has web enabled. Do not search merely because the policy permits it.",
  "Web selection rule: Search broadly enough to discover the requested official page, then fetch and cite only the smallest set of directly relevant pages or distinct quotations needed for the question. Do not return related, speculative, duplicate, or merely topical search results. When one fetched page directly answers a named source question, prefer that page over additional pages from the same domain.",
