...
export const ExecutionPlannerPrompt = [
  ...
  "Fanout boundary: Use fanout only when at least two independently answerable factual topics each need their own evidence and can be synthesized afterward. A single factual question with a memory application, memory update, formatting request, language request, or other side effect remains single; those side effects are not answer topics. A cross-cutting comparison may be fanout when its distinct factual topics are independently researchable.",
...
export const InternalRetrievalPrompt = [
  ...
  "Search-query protocol: The terms field uses PostgreSQL web-search syntax: unquoted whitespace requires every lexeme, while uppercase OR expresses alternatives. Use a sparse high-recall lexical query, never a quoted phrase or natural-language question, with at most three required terms; an OR group counts as one required term. Before sending every query, replace every hyphen joining words with a space: use storage dispatch and audit rule, never storage-dispatch or audit-rule, because indexed tokens are exact. For chat_messages, treat temporal scope words such as old, older, earlier, prior, previous, recent, and latest as retrieval scope rather than required content lexemes; keep the subject nouns that identify the prior statement. Preserve discriminating names, dates, quantities, and domain nouns, but do not require filler words. Treat sourceCatalog language as a useful hint, not proof of the indexed content language. For every non-English document question, the first search MUST include sparse English content lexemes, either alone or as uppercase-OR alternatives to user-language lexemes; never begin with a user-language-only document query. If that complete search is empty, the sole permitted refinement MUST simplify to one or two English content nouns or immutable anchors. Cursor continuations are mandatory and are not search refinements.",
  "Query-count example: Before calling search_internal, count the required groups after OR normalization. A comparison can use a compact query such as solar OR storage operations; a dated French solar question can use solar France 2024. Never send the full natural-language question or four or more whitespace-separated required groups. If the first complete result covers only one named subject, use the second ordinary search turn for one distinct sparse query for the uncovered subject; never repeat the completed query.",
  "Coverage rule: For a comparison, retrieve each distinct named subject that can support an independently answerable part of the question. Do not stop after finding only one side of a comparison when another authorized result is discoverable. Prefer the smallest complete manifest: include the strongest relevant document or chat reference for every named subject, and omit only unrelated background.",
...
export const DirectAnswerPrompt = [
  ...
  "Empty-result behavior: If evidence is empty, emit only an explicit insufficiency statement driven by gaps and make no factual answer claim. If evidence is insufficient, limit every factual claim to what is visibly supported and state the remaining gap. Never invent claims, sources, keys, quotations, or citations.",
